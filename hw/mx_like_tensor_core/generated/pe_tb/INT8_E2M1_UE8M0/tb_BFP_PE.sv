...
// Combo: INT8 x E2M1 / UE8M0   K=64 bs=16 vec=4
...
module tb_BFP_PE;
  ...
  localparam real GOLDEN = -0.107666015625;
  ...
  initial begin
    $dumpfile("tb_BFP_PE_INT8_E2M1_UE8M0.vcd");
    ...
    $display("[tb INT8_E2M1_UE8M0] accOut=%h  got=%f  golden=%f  relErr=%f%%",
             io_accOut, got, GOLDEN, relerr * 100.0);
    if (((got > GOLDEN ? got - GOLDEN : GOLDEN - got)) <= REL_TOL * (GOLDEN < 0 ? -GOLDEN : GOLDEN) + ABS_TOL)
      $display("[tb INT8_E2M1_UE8M0] RESULT: PASS");
    else
      $display("[tb INT8_E2M1_UE8M0] RESULT: FAIL");
