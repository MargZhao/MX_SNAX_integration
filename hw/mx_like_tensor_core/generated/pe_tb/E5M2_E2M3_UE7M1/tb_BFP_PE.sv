...
// Combo: E5M2 x E2M3 / UE7M1   K=64 bs=16 vec=4
...
module tb_BFP_PE;
  ...
  localparam real GOLDEN = -0.1262798309326172;
  ...
  initial begin
    $dumpfile("tb_BFP_PE_E5M2_E2M3_UE7M1.vcd");
    ...
    $display("[tb E5M2_E2M3_UE7M1] accOut=%h  got=%f  golden=%f  relErr=%f%%",
             io_accOut, got, GOLDEN, relerr * 100.0);
    if (((got > GOLDEN ? got - GOLDEN : GOLDEN - got)) <= REL_TOL * (GOLDEN < 0 ? -GOLDEN : GOLDEN) + ABS_TOL)
      $display("[tb E5M2_E2M3_UE7M1] RESULT: PASS");
    else
      $display("[tb E5M2_E2M3_UE7M1] RESULT: FAIL");
