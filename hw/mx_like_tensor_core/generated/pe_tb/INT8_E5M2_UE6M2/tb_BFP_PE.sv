...
// Combo: INT8 x E5M2 / UE6M2   K=64 bs=16 vec=4
...
module tb_BFP_PE;
  ...
  initial begin
    $dumpfile("tb_BFP_PE_INT8_E5M2_UE6M2.vcd");
    ...
    $display("[tb INT8_E5M2_UE6M2] accOut=%h  got=%f  golden=%f  relErr=%f%%",
             io_accOut, got, GOLDEN, relerr * 100.0);
    if (((got > GOLDEN ? got - GOLDEN : GOLDEN - got)) <= REL_TOL * (GOLDEN < 0 ? -GOLDEN : GOLDEN) + ABS_TOL)
      $display("[tb INT8_E5M2_UE6M2] RESULT: PASS");
    else
      $display("[tb INT8_E5M2_UE6M2] RESULT: FAIL");
