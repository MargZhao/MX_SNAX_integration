...
// Combo: E4M3 x E2M1 / UE4M3   K=64 bs=16 vec=4
...
module tb_BFP_PE;
  ...
  localparam real GOLDEN = -0.11591243743896484;
  ...
  initial begin
    $dumpfile("tb_BFP_PE_E4M3_E2M1_UE4M3.vcd");
    ...
    $display("[tb E4M3_E2M1_UE4M3] accOut=%h  got=%f  golden=%f  relErr=%f%%",
             io_accOut, got, GOLDEN, relerr * 100.0);
    if (((got > GOLDEN ? got - GOLDEN : GOLDEN - got)) <= REL_TOL * (GOLDEN < 0 ? -GOLDEN : GOLDEN) + ABS_TOL)
      $display("[tb E4M3_E2M1_UE4M3] RESULT: PASS");
    else
      $display("[tb E4M3_E2M1_UE4M3] RESULT: FAIL");
