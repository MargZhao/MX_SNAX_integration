...
// Combo: E2M3 x E2M1 / UE7M1   K=64 bs=16 vec=4
...
module tb_BFP_PE;
  ...
  localparam real GOLDEN = -0.130523681640625;
  ...
  initial begin
    $dumpfile("tb_BFP_PE_E2M3_E2M1_UE7M1.vcd");
    ...
    $display("[tb E2M3_E2M1_UE7M1] accOut=%h  got=%f  golden=%f  relErr=%f%%",
             io_accOut, got, GOLDEN, relerr * 100.0);
    if (((got > GOLDEN ? got - GOLDEN : GOLDEN - got)) <= REL_TOL * (GOLDEN < 0 ? -GOLDEN : GOLDEN) + ABS_TOL)
      $display("[tb E2M3_E2M1_UE7M1] RESULT: PASS");
    else
      $display("[tb E2M3_E2M1_UE7M1] RESULT: FAIL");
