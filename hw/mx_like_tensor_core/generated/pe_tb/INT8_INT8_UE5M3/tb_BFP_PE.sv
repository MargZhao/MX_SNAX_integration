...
// Combo: INT8 x INT8 / UE5M3   K=64 bs=16 vec=4
...
module tb_BFP_PE;
  ...
  localparam real GOLDEN = -0.12089484930038452;
  ...
  initial begin
    $dumpfile("tb_BFP_PE_INT8_INT8_UE5M3.vcd");
    ...
    $display("[tb INT8_INT8_UE5M3] accOut=%h  got=%f  golden=%f  relErr=%f%%",
             io_accOut, got, GOLDEN, relerr * 100.0);
    if (((got > GOLDEN ? got - GOLDEN : GOLDEN - got)) <= REL_TOL * (GOLDEN < 0 ? -GOLDEN : GOLDEN) + ABS_TOL)
      $display("[tb INT8_INT8_UE5M3] RESULT: PASS");
    else
      $display("[tb INT8_INT8_UE5M3] RESULT: FAIL");
